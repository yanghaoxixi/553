...
unique_id_file = divide_file.groupByKey().map(lambda s: [s[0],list(set(s[1]))]).filter(lambda s: len(s[1])>filter_threshold)
total_pair = unique_id_file.map(lambda s: sorted(s[1])).persist()
count_number = total_pair.count()
# phase1
frequent_candidate = total_pair.mapPartitions(lambda s: PCY(list(s),support,count_number)).distinct().collect()
# phase2
phase2_total_candidate = total_pair.mapPartitions(lambda s: total_candidate2(list(s),frequent_candidate))\
